...
import com.lwc.user.fallback.RoleResourceFallbackFactory;
...
@FeignClient(name = "eip-user-service", path = "/api/v1/roleResource", fallbackFactory = RoleResourceFallbackFactory.class)
public interface RoleResourceApi {

    @GetMapping(value = "/query/{id}")
    Result<RoleResourceBo> query(@PathVariable("id") Long id);
    ...
    Result<List<RoleResourceBo>> queryByIds(@RequestBody List<Long> ids);
    ...
    Result<List<RoleResourceBo>> query(@RequestBody RoleResourceBo entity);

    @PostMapping(value = "/create", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<RoleResourceBo> create(@RequestBody RoleResourceBo entity);

    @PostMapping(value = "/update", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<List<RoleResourceBo>> update(@RequestBody RoleResourceBo entity);

    @PostMapping(value = "/remove", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<Integer> remove(@RequestBody RoleResourceBo entity);

    @PostMapping(value = "/purge", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<Integer> purge(@RequestBody RoleResourceBo entity);

    @PostMapping(value = "/count", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<Integer> count(@RequestBody RoleResourceBo entity);

    @PostMapping(value = "/insertOrUpdateBatch", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<List<RoleResourceBo>> insertOrUpdateBatch(@RequestBody List<RoleResourceBo> entities);

    @PostMapping(value = "/page/{current}/{size}", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<IPage<RoleResourceBo>> page(@RequestBody RoleResourceBo entity, @PathVariable("current") int current, @PathVariable("size") int size);
}
