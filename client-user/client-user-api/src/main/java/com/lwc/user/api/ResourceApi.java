...
import com.lwc.user.fallback.ResourceFallbackFactory;
...
@FeignClient(name = "eip-user-service", path = "/api/v1/resource", fallbackFactory = ResourceFallbackFactory.class)
public interface ResourceApi {

    @GetMapping(value = "/query/{id}")
    Result<ResourceBo> query(@PathVariable("id") Long id);
    ...
    Result<List<ResourceBo>> queryByIds(@RequestBody List<Long> ids);
    ...
    Result<List<ResourceBo>> query(@RequestBody ResourceBo entity);

    @PostMapping(value = "/create", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<ResourceBo> create(@RequestBody ResourceBo entity);

    @PostMapping(value = "/update", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<List<ResourceBo>> update(@RequestBody ResourceBo entity);

    @PostMapping(value = "/remove", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<Integer> remove(@RequestBody ResourceBo entity);

    @PostMapping(value = "/purge", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<Integer> purge(@RequestBody ResourceBo entity);

    @PostMapping(value = "/count", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<Integer> count(@RequestBody ResourceBo entity);

    @PostMapping(value = "/insertOrUpdateBatch", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<List<ResourceBo>> insertOrUpdateBatch(@RequestBody List<ResourceBo> entities);

    @PostMapping(value = "/page/{current}/{size}", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    Result<IPage<ResourceBo>> page(@RequestBody ResourceBo entity, @PathVariable("current") int current, @PathVariable("size") int size);
}
