 ...
 * the License.
 */
package com.lwc.generator.engine;
...
import com.lwc.generator.config.ConstVal;
import com.lwc.generator.config.FileOutConfig;
import com.lwc.generator.config.GlobalConfig;
import com.lwc.generator.config.TemplateConfig;
import com.lwc.generator.config.builder.ConfigBuilder;
import com.lwc.generator.config.po.TableInfo;
import com.lwc.generator.config.rules.FileType;
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                // MpFeign.java
                if (null != tableInfo.getFeignName() && null != pathInfo.get(ConstVal.FEIGN_PATH)) {
                    String feignFile = String.format((pathInfo.get(ConstVal.FEIGN_PATH) + File.separator + tableInfo.getFeignName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.FEIGN, feignFile)) {
                        writer(objectMap, templateFilePath(template.getFeign()), feignFile);
                    }
                }
                // MpEntityExt.java
                if (null != tableInfo.getEntityExtName() && null != pathInfo.get(ConstVal.ENTITY_EXT_PATH)) {
                    String extitExtFile = String.format((pathInfo.get(ConstVal.ENTITY_EXT_PATH) + File.separator + tableInfo.getEntityExtName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.ENTITY_EXT, extitExtFile)) {
                        writer(objectMap, templateFilePath(template.getEntityExt()), extitExtFile);
                    }
                }
                // MpDao.java
                ...
                // MpFallBackFactory.java
                if (null != tableInfo.getFallbackName() && null != pathInfo.get(ConstVal.FALLBACK_PATH)) {
                    String fallbackFile = String.format((pathInfo.get(ConstVal.FALLBACK_PATH) + File.separator + tableInfo.getFallbackName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.FALLBACK, fallbackFile)) {
                        writer(objectMap, templateFilePath(template.getFallback()), fallbackFile);
                    }
                }

                // MpPayload.java
                if (null != tableInfo.getPayloadName() && null != pathInfo.get(ConstVal.PAYLOAD_PATH)) {
                    String payloadFile = String.format((pathInfo.get(ConstVal.PAYLOAD_PATH) + File.separator + tableInfo.getPayloadName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.PAYLOAD, payloadFile)) {
                        writer(objectMap, templateFilePath(template.getPayload()), payloadFile);
                    }
                }

                // MpHandler.java
                if (null != tableInfo.getHandlerName() && null != pathInfo.get(ConstVal.HANDLER_PATH)) {
                    String handlerFile = String.format((pathInfo.get(ConstVal.HANDLER_PATH) + File.separator + tableInfo.getHandlerName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.PAYLOAD, handlerFile)) {
                        writer(objectMap, templateFilePath(template.getHander()), handlerFile);
                    }
                }
            }
        } catch (Exception e) {
            logger.warn("无法创建文件，请检查配置信息！", e);
        }
        return this;
    }
